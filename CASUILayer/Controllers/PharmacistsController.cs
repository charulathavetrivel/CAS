...
namespace CASUILayer.Controllers
{
    public class PharmacistsController : Controller
    {
        ...
        public ActionResult CalculateDiscountedPrice(int medicineId)
        {
            var medicine = service.GetMedicineById(medicineId);
            float originalPrice = medicine.Price;
            float tax = medicine.Tax;
            float discountedPrice = service.CalculateDiscountedPrice(originalPrice, tax);
